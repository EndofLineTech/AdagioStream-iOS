...
@MainActor
public enum KeychainSyncMigrator {
    private static let log = OSLog(subsystem: "com.adagiostream.app.migration", category: "keychain-sync")
    private static let service = KeychainService.productionService

    /// Known account names that the app stores under the shared service.
    /// The migrator scans only these accounts; any item the app does not
    /// own remains untouched.
    private static let knownAccounts: [String] = [
        Constants.StorageKeys.providers
    ]

    /// Convenience entry point for production callers — uses
    /// `MigrationDiagnostics.shared`. Avoids the Swift 6 ban on referencing
    /// main-actor-isolated singletons in default-parameter values.
    public static func runIfNeeded() {
        runIfNeeded(diagnostics: .shared)
    }

    /// Runs the migration if it has not already completed.
    /// Safe to call from `ProviderManager.init` before any Keychain read.
    /// Tests may pass an isolated `MigrationDiagnostics` instance backed by
    /// a non-`.standard` `UserDefaults`.
    public static func runIfNeeded(
